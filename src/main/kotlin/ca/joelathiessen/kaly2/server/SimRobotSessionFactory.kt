...
class SimRobotSessionFactory(private val odoAngStdDev: Float, private val odoDistStdDev: Float,
                             ...
                             private val mapImage: BufferedImage, private val maxSensorRange: Float,
    ...
    private fun extractDataFromMapImage(mapImage: BufferedImage): MapImageData {
        ...
        for (xInc in 0 until mapImage.width) {
            for (yInc in 0 until mapImage.height) {
                if (mapImage.getRGB(xInc, yInc) == Color.BLACK.rgb) {
                    points.add(Point(xInc.toFloat(), yInc.toFloat()))
                    obsGrid[xInc][yInc] = Point(xInc.toFloat(), yInc.toFloat())
                } else if (mapImage.getRGB(xInc, yInc) == Color.RED.rgb) {
                    x = xInc.toFloat()
                    y = yInc.toFloat()
                } else if (mapImage.getRGB(xInc, yInc) == Color.GREEN.rgb) {
                    xEnd = xInc.toFloat()
                    yEnd = yInc.toFloat()
                }
            }

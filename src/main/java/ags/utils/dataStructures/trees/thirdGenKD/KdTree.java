...
public class KdTree<T> extends KdNode<T> {
    ...
    /* -------- Joel's copy on delete -------- */
    public KdTree<T> removePointAsCopy(double[] point, T value) {
        KdTree<T> newTree = new KdTree<T>(this);
        removePointAsCopy(newTree, point, value);
        return newTree;
    }
    ...
    protected static <T> void nearestNeighborSearchStep (
        ...
        while (!cursor.isLeaf()) {
            ...
            if(pathNotTaken.size > 0) { // Joel's empty bucket noticer
        ...
        if (cursor.size > 0) { // Joel's empty bucket noticer
